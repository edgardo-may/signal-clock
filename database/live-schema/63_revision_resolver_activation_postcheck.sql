-- Phase 63: read-only verification after a human-approved tenant flag change.
...
SELECT jsonb_build_object(
  'phase','63_revision_resolver_activation_postcheck',
  'read_only',current_setting('transaction_read_only'),
  'pilot_feature',COALESCE((SELECT jsonb_agg(jsonb_build_object('mode',mode,'enabled',enabled)) FROM features f CROSS JOIN expected e WHERE f.cliente_id=e.cliente_id),'[]'::jsonb),
  'other_active_tenants',COALESCE((SELECT jsonb_agg(f.cliente_id) FROM features f CROSS JOIN expected e WHERE f.mode='ACTIVE' AND f.enabled AND f.cliente_id<>e.cliente_id),'[]'::jsonb),
  ...
  'postcheck_pass',
    EXISTS(SELECT 1 FROM features f CROSS JOIN expected e WHERE f.cliente_id=e.cliente_id AND f.mode='ACTIVE' AND f.enabled)
    AND NOT EXISTS(SELECT 1 FROM features f CROSS JOIN expected e WHERE f.cliente_id<>e.cliente_id AND f.mode='ACTIVE' AND f.enabled)
...
) AS revision_resolver_activation_postcheck
