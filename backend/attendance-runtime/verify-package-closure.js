...
function verifyPackageClosure() {
  ...
  assert.match(dockerfile, /ATTENDANCE_RUNTIME_CAPABILITY=\$ATTENDANCE_RUNTIME_CAPABILITY/)
  ...
  assert.match(read('backend/attendance-runtime/app.js'), /\/internal\/attendance\/active/)
  assert.doesNotMatch(service, /WorkdayPersistenceService|PERSIST_MODE/)
  assert.match(cloudBuild, /BUILD_SHA=\$\{_BUILD_SHA\}/)
  assert.match(cloudBuild, /ATTENDANCE_RUNTIME_CAPABILITY=\$\{_ATTENDANCE_RUNTIME_CAPABILITY\}/)
  assert.match(cloudBuild, /publish-immutable-runtime-image/)
  assert.match(cloudBuild, /args: \['push','\$\{_IMAGE_URI\}:\$\{_IMAGE_TAG\}'\]/)
  ...
  assert.match(cloudBuild, /ATTENDANCE_RUNTIME_CAPABILITY=\$expected_capability/)
  assert.match(cloudBuild, /attendance_container_build_validation=PASS/)
  assert.doesNotMatch(cloudBuild, /gcloud run|images:/i)
  ...
  assert.match(cloudShellBuild, /attendance-runtime-v2/)
  assert.match(cloudShellBuild, /dockerConfig\.immutableTags/)
  assert.match(cloudShellBuild, /gcloud artifacts docker images describe/)
  assert.doesNotMatch(cloudShellBuild, /gcloud run/i)
  return {
